...
for i = 1:numSolutions
    %do optimization
    currentIteration = i;
    options = optimoptions(@fmincon,'Display','iter','MaxFunEvals',1500,'Algorithm','interior-point');
    [answer,fval,exitflag]=fmincon(@(q0)criterionPart4(q0,target,link_length,min_roll,min_pitch,min_yaw,max_roll,max_pitch,max_yaw, answers, currentIteration),q0,[],[],[],[],lb,ub,@(q0)constraints(q0,link_length,target,obstacles),options);
    answers(:,:,i+1) = answer;
    fvals(i) = fval;
end
% disp(fvals);
